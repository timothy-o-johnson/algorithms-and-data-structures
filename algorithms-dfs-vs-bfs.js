...
class BinarySearchTree {
  ...
  traverseInOrder(list, currentNode) {
    // get down to smallest node add that node
  ...
  validateDFS(list, currentNode) {
    return modifiedTraverseInOrder(list, currentNode)[0] === -1 ? false : true

    function modifiedTraverseInOrder(list, currentNode) {
      if (list[0] === -1) {
        list = -1
        return -1
      }
      // go down to the smallest node on the left
      if (currentNode.left !== null) {
        modifiedTraverseInOrder(list, currentNode.left)
      }

      // add current node
      // if list first element is -1, return -1
      if (list[0] === -1) {
        list = -1
        return -1
      }
      // else if currentNode.value is greater than or equal to the last element, add element
      else if (list.length === 0 || currentNode.value >= list[list.length - 1]) {
        list.push(currentNode.value)
      } else {
        // else replace list with [-1]
        list[0] = -1

        // return list
      }

      // add smallest node on the right
      if (currentNode.right !== null) {
        modifiedTraverseInOrder(list, currentNode.right)
      }
  ...
  isValidBST(root, upperBound = Infinity, lowerBound = -Infinity) {
    if (!root) {
      return true
    }
    ...
    return (
      this.isValidBST(root.left, Math.min(upperBound, root.val), lowerBound) &&
      this.isValidBST(root.right, upperBound, Math.max(lowerBound, root.val))
    )
  }
}
...
console.log(tree.traverseInOrder([], tree.root))
console.log(tree.validateDFS([], tree.root))
