class HashTable {
  ...
  set (key, value) {
    ...
    if (this.data[hashAddress]) {
      // push new key-value
      this.data[hashAddress] = this.data[hashAddress].push([key, value])
    } else {
  ...
  get (key) {
    ...
    if (hashData && hashData.length === 1) {
      return this.data[hashAddress][0][1]
    } else if (hashData && hashData.length > 1) {
      ...
      for (var i = 0; i < hashArray.length; i++) {
        ...
        if (hashArrayKey === key) return hashArray[i]
      }
    } else {
      return undefined
    }
